...
@click.option("--display", default=None,  help="Display name shown in Power BI (defaults to --name).")
@click.option("--guid",    default=None,  help="Visual GUID (auto-generated if omitted).")
@click.option("--output",  default=".",   type=click.Path(), help="Parent directory for the new project.")
...
def custom_visual_scaffold(
    ...
    # pbiviz.json
    (project_dir / "pbiviz.json").write_text(json.dumps({
        "visual": {
            "name": name,
    ...
    # capabilities.json
    (project_dir / "capabilities.json").write_text(json.dumps({
    ...
    # src/visual.ts
    (project_dir / "src" / "visual.ts").write_text(f'''\
"use strict";
...
''', encoding="utf-8")
    ...
    console.print(f"  pbi custom-visual build --path .")
    console.print(f"  pbi custom-visual package --path .")
...
@click.option("--path",   default=".",   type=click.Path(exists=True), help="Visual project directory.")
@click.option("--output", default=None,  type=click.Path(), help="Output .pbiviz path (default: dist/).")
...
@click.option("--pbiviz", required=True, type=click.Path(exists=True),

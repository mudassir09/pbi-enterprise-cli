...
def run_doctor(output_json: bool) -> None:
    ...
    try:
        ...
        checks.append({"check": "pythonnet", "status": "fail", "detail": "Not installed (Windows only)"})

    # sqlalchemy
    try:
        import sqlalchemy
        checks.append({"check": "sqlalchemy [sources]", "status": "pass", "detail": sqlalchemy.__version__})
    except ImportError:
        checks.append({"check": "sqlalchemy [sources]", "status": "warn", "detail": "Not installed (optional)"})

    # fastapi
    try:
        import fastapi
        checks.append({"check": "fastapi [server]", "status": "pass", "detail": fastapi.__version__})
    except ImportError:
        checks.append({"check": "fastapi [server]", "status": "warn", "detail": "Not installed (optional)"})

...
VISUAL_RULES = [
    ({"revenue", "sales", "amount", "total"}, "Clustered Bar Chart", "Good for comparing amounts across categories"),
        ...
        for keywords, visual_type, rationale in VISUAL_RULES:
            if any(kw in measure_text for kw in keywords):
                recommendations.append({
                    "visual": visual_type,
                    "rationale": rationale,
                    "matchedMeasures": [m for m in measures if any(kw in m.lower() for kw in keywords)],
                })

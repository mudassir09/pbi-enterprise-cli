...
- **Fabric IQ ontology (preview)** *(added 2026-06-12; `fabric ontology` + `govern ai-readiness` shipped)* — Fabric IQ exposes ontology items (entity types, properties, relationship types generated from a semantic model) as the semantic layer for AI agents. The CLI angle is twofold: (1) manage ontology items via the standard item REST surface (`pbi fabric ontology list/get/create/update/delete`), and (2) make models *generate good ontologies* — an AI-readiness audit (`pbi govern ai-readiness`) covering descriptions, hidden key columns, marked date table, no auto date/time tables, no Decimal columns (the IQ graph returns nulls for Decimal), and relationship coverage. Generation from a semantic model is portal-only today; wrap it when an API ships. Data bindings require Direct Lake + managed lakehouse tables, which pairs with `pbi migrate direct-lake`.
